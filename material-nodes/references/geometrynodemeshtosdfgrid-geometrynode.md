# Geometrynodemeshtosdfgrid Geometrynode, Geometrynodemeshtovolume Geometrynode, Geometrynodemeshuvsphere Geometrynode, Geometrynodeobjectinfo Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeMeshToSDFGrid(GeometryNode); GeometryNodeMeshToVolume(GeometryNode); GeometryNodeMeshUVSphere(GeometryNode); GeometryNodeObjectInfo(GeometryNode); GeometryNodeOffsetCornerInFace(GeometryNode); GeometryNodeOffsetPointInCurve(GeometryNode); GeometryNodePoints(GeometryNode).
...
Build a signed-distance volume grid out of a mesh.
...
Build a fog volume that follows the shape of the supplied mesh surface.
...
Build a spherical mesh from quad faces, capped by triangles at the poles.
...
Pull data out of an object.

transform_space 

Sets the frame the vector and geometry outputs are expressed in (default 'ORIGINAL' )

- ORIGINAL
Original – Give back the geometry in the input object's own transform, with location, rotation and scale measured against the world origin.

- RELATIVE
Relative – Pull the input object's geometry, location, rotation and scale into the modified object while preserving the two objects' relative placement in the scene.
...
Look up other corners that belong to the same face as a given corner.
...
Shift a control-point index by a step while staying inside its curve.
...
Build a point cloud whose per-point positions and radii are driven by fields
