# Geometrynodesdfgridmeancurvature Geometrynode, Geometrynodesdfgridmedian Geometrynode, Geometrynodesdfgridoffset Geometrynode, Geometrynodeselfobject Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeSDFGridMeanCurvature(GeometryNode); GeometryNodeSDFGridMedian(GeometryNode); GeometryNodeSDFGridOffset(GeometryNode); GeometryNodeSelfObject(GeometryNode); GeometryNodeSeparateComponents(GeometryNode); GeometryNodeSeparateGeometry(GeometryNode); GeometryNodeSetCurveHandlePositions(GeometryNode).
...
Smooth a signed distance field with mean curvature flow. The surface evolves according to its mean curvature, so highly curved areas are smoothed more strongly than flat ones
...
Run a median filter over a signed distance field. Cuts down noise yet keeps the field's sharp features and edges intact
...
Shift a signed distance field surface outward or inward by a world-space distance. A positive value dilates and a negative value erodes, all while keeping the signed distance property intact
...
Get the object that holds the geometry nodes modifier that is running right now
...
Break a geometry apart, giving each data type within it its own output
...
Use a selection to divide a geometry into two separate geometry outputs
...
Assign new positions to the handles on Bézier curves
...
Literal[Node Geometry Curve Handle Side Items]
