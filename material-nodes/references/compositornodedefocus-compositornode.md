# Compositornodedefocus Compositornode, Compositornodedenoise Compositornode, Compositornodedespeckle Compositornode, Compositornodediffmatte Compositornode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: CompositorNodeDefocus(CompositorNode); CompositorNodeDenoise(CompositorNode); CompositorNodeDespeckle(CompositorNode); CompositorNodeDiffMatte(CompositorNode); CompositorNodeDilateErode(CompositorNode); CompositorNodeDisplace(CompositorNode); CompositorNodeDistanceMatte(CompositorNode).
...
Adds 2D depth-of-field blur, driven by a Z depth map or a mask.

angle

Rotation offset applied to the bokeh shape (in [0, 1.5708], default 0.0)
...
Cap on the blur, the largest allowed CoC radius (in [0, 10000], default 0.0)
...
Strength of the focal blur; 128 (infinity) gives sharp focus, and halving it doubles the blur radius (in [0, 128], default 0.0)
...
Scene whose active camera is used (falls back to the render scene when left unset)
...
Turn off when feeding an image in place of a real z-buffer (switched on automatically for non-image-based nodes such as a time node) (default True)
...
Scales the Z input when no z-buffer is used, setting the cap on blur marked by white or an input value of 1 (in [0, 1000], default 0.0)
...
Cleans up noise in renders coming from Cycles and other ray tracers.
...
Cleans up visibly noisy patches of an image while leaving busy, detailed regions alone.
...
Builds a matte that separates foreground from a scene by differencing it against a known background plate.
...
Grows or trims back the edges of a mask.
...
Shifts each pixel's location according to an offset vector.
...
Generates a matte by measuring how far apart colors sit in 3D color space.
