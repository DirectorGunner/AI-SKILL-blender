# Geometrynodeuvunwrap Geometrynode, Geometrynodevertexofcorner Geometrynode, Geometrynodeviewer Geometrynode, Geometrynodeviewporttransform Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeUVUnwrap(GeometryNode); GeometryNodeVertexOfCorner(GeometryNode); GeometryNodeViewer(GeometryNode); GeometryNodeViewportTransform(GeometryNode); GeometryNodeVolumeCube(GeometryNode); GeometryNodeVolumeToMesh(GeometryNode); GeometryNodeWarning(GeometryNode).
...
Builds a UV map by following the edges marked as seams.
...
Looks up which vertex a given face corner connects to.
...
Sends the connected data to the Spreadsheet Editor so it can be inspected.
...
The domain over which fields get evaluated (default 'POINT' )

Type :

Literal[Attribute Domain With Auto Items]
...
Reports the viewing direction and position of the 3D viewport.
...
Builds a dense volume in which a field drives each voxel’s density according to where that voxel sits in the grid.
...
Builds a mesh that follows the “surface” of a volume.
...
Lets a node group raise its own custom warnings.
