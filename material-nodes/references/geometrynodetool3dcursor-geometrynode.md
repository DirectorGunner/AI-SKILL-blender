# Geometrynodetool3Dcursor Geometrynode, Geometrynodetoolactiveelement Geometrynode, Geometrynodetoolfaceset Geometrynode, Geometrynodetoolmouseposition Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeTool3DCursor(GeometryNode); GeometryNodeToolActiveElement(GeometryNode); GeometryNodeToolFaceSet(GeometryNode); GeometryNodeToolMousePosition(GeometryNode); GeometryNodeToolSelection(GeometryNode); GeometryNodeToolSetFaceSet(GeometryNode); GeometryNodeToolSetSelection(GeometryNode).
...
The position and orientation of the scene's 3D cursor.
...
The active element indices of the geometry being edited, supplied during tool execution.
...
The sculpt face set value assigned to each face.
...
Reads back where the mouse cursor currently sits.
...
Exposes whatever the user has selected on the geometry being edited so a tool run can act on it.
...
Writes sculpt face-set values onto the faces it is given.
...
During a tool run, writes the selection state back onto the geometry being edited.
...
Literal[Attribute Domain Point Edge Face Curve Items]
...
- BOOLEAN
Boolean – Keeps the selection as true/false values while in edit mode.

- FLOAT
Float – Keeps the selection as floating-point values; on mesh geometry these are held inverted and used as the sculpt-mode mask.
