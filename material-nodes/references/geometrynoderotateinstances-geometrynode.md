# Geometrynoderotateinstances Geometrynode, Geometrynodesamplecurve Geometrynode, Geometrynodesamplegrid Geometrynode, Geometrynodesamplegridindex Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeRotateInstances(GeometryNode); GeometryNodeSampleCurve(GeometryNode); GeometryNodeSampleGrid(GeometryNode); GeometryNodeSampleGridIndex(GeometryNode); GeometryNodeSampleIndex(GeometryNode); GeometryNodeSampleNearest(GeometryNode); GeometryNodeSampleNearestSurface(GeometryNode).
...
Turn geometry instances around, using either local or global space
...
Read data at a point that lies a given distance along a curve from its start
...
- FACTOR
Factor – Locate sample positions along the curve as a fraction of its full length.

- LENGTH
Length – Locate sample positions along the curve as a distance measured from its start.
...
Base the sample lengths on the combined length of every curve instead of measuring within each selected curve (default False)
...
Read values out of the chosen volume grid
...
Read out the values held by particular voxels of a volume grid
...
Read values out of particular elements of a geometry

clamp

Restrict indices to the bounds of the attribute domain rather than returning a default value when an index is invalid (default False)
...
Locate the geometry element nearest to a given position. Works much like the “Index of Nearest” node
...
Literal[Attribute Domain Only Mesh Items]
...
Work out the interpolated value a mesh attribute takes at the surface point closest to the input
