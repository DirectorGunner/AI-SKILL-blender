# Shadernodevolumecoefficients Shadernode, Shadernodevolumeinfo Shadernode, Shadernodevolumeprincipled Shadernode, Shadernodevolumescatter Shadernode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: ShaderNodeVolumeCoefficients(ShaderNode); ShaderNodeVolumeInfo(ShaderNode); ShaderNodeVolumePrincipled(ShaderNode); ShaderNodeVolumeScatter(ShaderNode); ShaderNodeWavelength(ShaderNode); ShaderNodeWireframe(ShaderNode); SimulationStateItem(bpy_struct).
...
Represents all three volumetric physical processes through their coefficients.

phase

Phase function used for light that scatters (default '`HENYEY_GREENSTEIN`' )

- `HENYEY_GREENSTEIN`
Henyey-Greenstein – The standard default; models how light scatters via the Henyey-Greenstein term.

- `FOURNIER_FORAND`
Fournier-Forand – A Fournier-Forand model, typically chosen for light scattering underwater.

- DRAINE
Draine – The Draine family of phase functions, most often applied to scattering within interstellar dust.

- RAYLEIGH
Rayleigh – A Rayleigh model, suited to particles below the light's wavelength, like sunlight scattering through earth’s atmosphere.

- MIE
Mie – An approximate Mie response from water droplets, handy for scattering inside clouds and fog.
...
Pulls volume data attributes out of volume grids.
...
Rolls every volume-shading component into one convenient node.
...
Deflects light passing through the volume; a common way to add fog to a scene.
...
Maps a wavelength figure onto a matching RGB color.
...
Returns the edges of an object the way Cycles sees them.
Note: because meshes are triangulated ahead of Cycles processing, the topology always reads as triangulated.

use_pixel_size

Measure in screen pixels rather than world units (default False)
...
Which attribute domain holds this value in the simulation state (default 'POINT' )
...
| GeometryNodeSimulationOutput.active_item GeometryNodeSimulationOutput.state_items | NodeGeometrySimulationOutputItems.new NodeGeometrySimulationOutputItems.remove |
