# Compositornodebokehblur Compositornode, Compositornodebokehimage Compositornode, Compositornodeboxmask Compositornode, Compositornodebrightcontrast Compositornode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: CompositorNodeBokehBlur(CompositorNode); CompositorNodeBokehImage(CompositorNode); CompositorNodeBoxMask(CompositorNode); CompositorNodeBrightContrast(CompositorNode); CompositorNodeChannelMatte(CompositorNode); CompositorNodeChromaMatte(CompositorNode); CompositorNodeColorBalance(CompositorNode).
...
Produce a bokeh-style blur akin to Defocus; unlike Defocus, the in-focus region here is set within the compositor
...
Produce an image carrying a bokeh shape, meant to feed the Bokeh Blur filter node
...
Build a rectangular mask handy for use as a basic matte
...
Tune an image's brightness and contrast
...
Build a matte from the differences between color channels
...
Build a matte from chroma values
...
The color that white is remapped onto; temperature and tint are derived back and forth from it without manual steps (array of 3 items, in [0, inf], default (0.0, 0.0, 0.0))
