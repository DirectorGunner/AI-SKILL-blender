# Compositornodelumamatte Compositornode, Compositornodemapuv Compositornode, Compositornodemask Compositornode, Compositornodemasktosdf Compositornode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: CompositorNodeLumaMatte(CompositorNode); CompositorNodeMapUV(CompositorNode); CompositorNodeMask(CompositorNode); CompositorNodeMaskToSDF(CompositorNode); CompositorNodeMovieClip(CompositorNode); CompositorNodeMovieDistortion(CompositorNode); CompositorNodeNormal(CompositorNode).
...
Generates a matte by keying on differences in luminance, or brightness.
...
Reprojects a texture through UV coordinates so it can be laid onto objects during compositing.
...
Brings in a mask defined by a mask data-block that was authored within the image editor.
...
Derives a signed distance field out of a supplied mask.
...
Pulls a still or video frame from a movie clip data-block, a common source when working with motion tracking.
...
Strips lens distortion out of footage by drawing on the camera lens parameters recorded during motion tracking.
...
Feeds normalized normal vectors onward to the rest of the nodes in the tree.
