# Shadernodetexchecker Shadernode, Shadernodetexcoord Shadernode, Shadernodetexenvironment Shadernode, Shadernodetexgabor Shadernode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: ShaderNodeTexChecker(ShaderNode); ShaderNodeTexCoord(ShaderNode); ShaderNodeTexEnvironment(ShaderNode); ShaderNodeTexGabor(ShaderNode); ShaderNodeTexGradient(ShaderNode); ShaderNodeTexIES(ShaderNode).
...
A procedural texture that produces a checkerboard.
...
Provides several kinds of texture coordinates at once.
Most often wired into the inputs of texture nodes
...
Reads an image file as an environment texture, most often paired with the Background node to light a scene.
...
Outputs blended color and intensity values derived from the incoming vector.
...
Reproduces real-world fixtures from IES files, which record how a light source spreads its intensity in each direction.
