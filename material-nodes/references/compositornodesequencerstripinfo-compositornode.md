# Compositornodesequencerstripinfo Compositornode, Compositornodesetalpha Compositornode, Compositornodesplit Compositornode, Compositornodestabilize Compositornode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: CompositorNodeSequencerStripInfo(CompositorNode); CompositorNodeSetAlpha(CompositorNode); CompositorNodeSplit(CompositorNode); CompositorNodeStabilize(CompositorNode); CompositorNodeSwitch(CompositorNode); CompositorNodeSwitchView(CompositorNode); CompositorNodeTime(CompositorNode).
...
Reports details about the modifier's currently active strip.
...
Attaches an alpha channel to an image.
...
Joins two images so they sit next to each other; most often paired with a Viewer node.
...
Steadies footage by relying on the 2D stabilization parameters from motion tracking.
...
Flips between a pair of images via a checkbox toggle.
...
Merges the left and right views together into one stereo 3D result.
...
Produces a factor running from 0.0 to 1.0 across the scene's start and end frames, shaped by a curve mapping.
