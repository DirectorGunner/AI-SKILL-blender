# Shadernodehuesaturation Shadernode, Shadernodeinvert Shadernode, Shadernodelayerweight Shadernode, Shadernodelightfalloff Shadernode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: ShaderNodeHueSaturation(ShaderNode); ShaderNodeInvert(ShaderNode); ShaderNodeLayerWeight(ShaderNode); ShaderNodeLightFalloff(ShaderNode); ShaderNodeLightPath(ShaderNode); ShaderNodeMapping(ShaderNode); ShaderNodeMapRange(ShaderNode).
...
Adjust color using the HSV color model
...
Flip a color into its negative
...
Output a mix factor based on the angle between the surface normal and the viewing direction.
Frequently used to stack shaders via the Mix Shader node
...
Control the rate at which light intensity drops with distance. Mostly serves non-physical effects, since real light always falls off quadratically
...
Read which kind of incoming ray is currently triggering the shader.
A common use is non-physical shading tricks
...
Reposition the input vector through combined translation, rotation, and scaling

vector_type 

Kind of vector the mapping operates on (default 'POINT' )
...
Convert a value from one range onto a destination range

clamp 

Restrict the output to the target range [To Min, To Max] (default False)
