# Functionnodehashvalue Functionnode, Functionnodeinputbool Functionnode, Functionnodeinputcolor Functionnode, Functionnodeinputint Functionnode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: FunctionNodeHashValue(FunctionNode); FunctionNodeInputBool(FunctionNode); FunctionNodeInputColor(FunctionNode); FunctionNodeInputInt(FunctionNode); FunctionNodeInputRotation(FunctionNode); FunctionNodeInputSpecialCharacters(FunctionNode); FunctionNodeInputString(FunctionNode).
...
Produce a pseudo-random integer by treating the supplied input as the seed
...
Supply a True/False value that downstream nodes in the tree can read

boolean 

The value applied when the socket has no incoming link (default False)
...
Emit a color value picked through the color picker widget
...
Supply an integer value that downstream nodes in the tree can read

integer 

The value applied when the socket has no incoming link (in [-inf, inf], default 1)
...
Supply a rotation value that downstream nodes in the tree can read

rotation_euler 

The value applied when the socket has no incoming link (array of 3 items, in [-inf, inf], default (0.0, 0.0, 0.0))
...
Emit string characters that the keyboard cannot enter directly
...
Supply a string value that downstream nodes in the tree can read
