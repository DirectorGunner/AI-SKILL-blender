# Shadernodeemission Shadernode, Shadernodefloatcurve Shadernode, Shadernodefresnel Shadernode, Shadernodegamma Shadernode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: ShaderNodeEmission(ShaderNode); ShaderNodeFloatCurve(ShaderNode); ShaderNodeFresnel(ShaderNode); ShaderNodeGamma(ShaderNode); ShaderNodeGroup(ShaderNode); ShaderNodeHairInfo(ShaderNode); ShaderNodeHoldout(ShaderNode).
...
Emission shader of the Lambertian type
...
Send an input float through a curve and return the resulting float
...
Output a mix factor derived from the angle between the surface normal and the viewing direction via the Fresnel equations.
A common use is blending in reflections toward grazing angles
...
Run a gamma correction on the input
...
class bpy.types. ShaderNodeGroup ( ShaderNode ) 
...
Read out information about a hair curve
...
Punch a zero-alpha “hole” into the image, handy when compositing.
Note: this shader can only generate alpha if transparency is turned on in the film settings
