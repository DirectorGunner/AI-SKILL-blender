# Geometrynodeinputnormal Geometrynode, Geometrynodeinputobject Geometrynode, Geometrynodeinputposition Geometrynode, Geometrynodeinputradius Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeInputNormal(GeometryNode); GeometryNodeInputObject(GeometryNode); GeometryNodeInputPosition(GeometryNode); GeometryNodeInputRadius(GeometryNode); GeometryNodeInputSceneTime(GeometryNode); GeometryNodeInputShadeSmooth(GeometryNode); GeometryNodeInputShortestEdgePaths(GeometryNode).
...
Read out, at each element, a unit-length vector pointing outward from the geometry's surface.

legacy_corner_normals

Fall back to face normals on the face corner domain, reproducing the node's former behavior (default False)
...
Read out a vector giving where each element is positioned.
...
Read out the radius value carried at each point of curve or point-cloud geometry.
...
Read out where the scene's animation currently sits, expressed either as frames or as seconds.
...
Report, per face, whether its normals are set to render smooth or to stay sharp.
...
Trace the cheapest routes through a mesh's edges toward chosen target vertices, letting you set the cost weighting for each edge.
