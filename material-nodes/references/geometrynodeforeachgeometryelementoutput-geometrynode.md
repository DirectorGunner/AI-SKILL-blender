# Geometrynodeforeachgeometryelementoutput Geometrynode, Geometrynodegeometrytoinstance Geometrynode, Geometrynodegetgeometrybundle Geometrynode, Geometrynodegetnamedgrid Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeForeachGeometryElementOutput(GeometryNode); GeometryNodeGeometryToInstance(GeometryNode); GeometryNodeGetGeometryBundle(GeometryNode); GeometryNodeGetNamedGrid(GeometryNode); GeometryNodeGizmoDial(GeometryNode); GeometryNodeGizmoLinear(GeometryNode); GeometryNodeGizmoTransform(GeometryNode).
...
class bpy.types. GeometryNodeForeachGeometryElementOutput ( GeometryNode ) 
...
The geometry domain that the loop runs across (default 'POINT' )
...
NodeGeometryForeachGeometryElementGenerationItems[ForeachGeometryElementGenerationItem]
...
The iteration index referenced by inspection tools such as the viewer node or socket inspection (in [-inf, inf], default 0)
...
Turn each input geometry into an instance, which for large inputs can run far quicker than the Join Geometry node
...
Fetch the bundle attached to a geometry
...
Pull a named volume grid out of a volume geometry
...
Display a dial gizmo over a value in the viewport
...
Literal[Geometry Nodes Gizmo Color Items]
...
Display a linear gizmo over a value in the viewport
...
Literal[Geometry Nodes Linear Gizmo Draw Style Items]
...
Display a transform gizmo in the viewport
