# Shadernodecameradata Shadernode, Shadernodeclamp Shadernode, Shadernodecombinecolor Shadernode, Shadernodecombinexyz Shadernode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: ShaderNodeCameraData(ShaderNode); ShaderNodeClamp(ShaderNode); ShaderNodeCombineColor(ShaderNode); ShaderNodeCombineXYZ(ShaderNode); ShaderNodeCustomGroup(ShaderNode); ShaderNodeDisplacement(ShaderNode); ShaderNodeEeveeSpecular(ShaderNode).
...
Read out camera data and how it relates to where the current shading point sits
...
Hold a value within a chosen lower and upper bound
...
Assemble a color out of its separate channels, with a choice of color models

mode 

Color model used while combining (default 'RGB' )
...
Build a vector out of separate X, Y, and Z components
...
A user-defined Shader Group Node intended for Python-defined nodes
...
Push the surface outward along its normal direction

space 

Coordinate space the input height is measured in (default 'OBJECT' )

- OBJECT
Object Space – Height is read in object space, so it scales with the object.

- WORLD
World Space – Height is read in world space, independent of object scale.
...
Comparable to the Principled BSDF, but driven by a specular rather than metallic workflow: you supply the facing reflection color directly. Because energy is not conserved, results can deviate from physical accuracy
