# Functionnodecombinematrix Functionnode, Functionnodecombinetransform Functionnode, Functionnodecompare Functionnode, Functionnodeeulertorotation Functionnode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: FunctionNodeCombineMatrix(FunctionNode); FunctionNodeCombineTransform(FunctionNode); FunctionNodeCompare(FunctionNode); FunctionNodeEulerToRotation(FunctionNode); FunctionNodeFindInString(FunctionNode); FunctionNodeFloatToInt(FunctionNode); FunctionNodeFormatString(FunctionNode).
...
Build a 4x4 matrix out of its separate component values
...
Merge a translation vector, a rotation, and a scale vector into one transformation matrix
...
Run a comparison between the two supplied inputs
...
Literal[Node Compare Operation Items]
...
Assemble a rotation from individual angles taken about each axis
...
Count how many times one string appears inside another and report where the first hit starts
...
Turn a floating-point number into an integer, letting you pick the conversion method

rounding_mode 

How the float gets reduced to an integer (default 'ROUND' )

Type :

Literal[Node Float To Int Items]
...
Substitute values into a string using formatting syntax compatible with Python and path templates
