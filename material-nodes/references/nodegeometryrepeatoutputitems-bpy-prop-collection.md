# Nodegeometryrepeatoutputitems Bpy Prop Collection, Nodegeometrysimulationoutputitems Bpy Prop Collection, Nodegeometryvieweritem Bpy Struct, Nodegeometryvieweritems Bpy Prop Collection ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: NodeGeometryRepeatOutputItems(bpy_prop_collection); NodeGeometrySimulationOutputItems(bpy_prop_collection); NodeGeometryViewerItem(bpy_struct); NodeGeometryViewerItems(bpy_prop_collection); NodeGetBundleItem(NodeInternal); NodeGroup(NodeInternal); NodeGroupInput(NodeInternal); NodeGroupOutput(NodeInternal); NodeInputs(bpy_prop_collection); NodeInstanceHash(bpy_struct).
...
A set holding the repeat items for this node.
...
item (RepeatItem) – Item, The item to remove (never None)
...
A set holding the simulation items for this node.
...
item (SimulationStateItem) – Item, The item to remove (never None)
...
Drop the item on its own once nothing links to it (default False)
...
| GeometryNodeViewer.active_item GeometryNodeViewer.viewer_items | NodeGeometryViewerItems.new NodeGeometryViewerItems.remove |
...
A set holding the viewer items for this node.
...
item (NodeGeometryViewerItem) – Item, The item to remove (never None)
...
Fetch a single bundle entry given its path.

socket_type

When the type differs, the value may be coerced automatically (default 'FLOAT' )
...
class bpy.types. NodeGroup ( NodeInternal )
...
Surface data wired in from within a node group as inputs on its interface
...
Emit data out of a node group from inside it

is_active_output

True when this node serves as the group's active output (default False)
...
class bpy.types. NodeInstanceHash ( bpy_struct )

Hash map storing per-node-instance data.
