# Geometrynodecornersofedge Geometrynode, Geometrynodecornersofface Geometrynode, Geometrynodecornersofvertex Geometrynode, Geometrynodecubegridtopology Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeCornersOfEdge(GeometryNode); GeometryNodeCornersOfFace(GeometryNode); GeometryNodeCornersOfVertex(GeometryNode); GeometryNodeCubeGridTopology(GeometryNode); GeometryNodeCurveArc(GeometryNode); GeometryNodeCurveEndpointSelection(GeometryNode); GeometryNodeCurveHandleTypeSelection(GeometryNode).
...
Look up the face corners attached to each edge.
...
Look up which corners belong to a given face.
...
Look up the face corners attached to each vertex.
...
Build a boolean grid topology at the requested dimensions, intended to feed the Field to Grid node.
...
Build an arc as a poly spline.

mode

Selects how the radius and position of the arc are worked out (default 'RADIUS' )

- POINTS
Points – Lay the arc through 3 points on a circle; the curve runs between the start and end of those points.
...
Yield a selection covering any chosen count of endpoints within each spline.
...
Yield a selection driven by the handle types assigned to Bézier control points.
...
Selects whether the left handle, the right handle, or both get tested (default set())
