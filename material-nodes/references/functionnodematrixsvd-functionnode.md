# Functionnodematrixsvd Functionnode, Functionnodeprojectpoint Functionnode, Functionnodequaterniontorotation Functionnode, Functionnoderandomvalue Functionnode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: FunctionNodeMatrixSVD(FunctionNode); FunctionNodeProjectPoint(FunctionNode); FunctionNodeQuaternionToRotation(FunctionNode); FunctionNodeRandomValue(FunctionNode); FunctionNodeReplaceString(FunctionNode); FunctionNodeRotateEuler(FunctionNode); FunctionNodeRotateRotation(FunctionNode).
...
Performs a singular value decomposition on a matrix's inner 3x3 block.
...
Sends a point through a matrix, accounting for location, rotation, scale, and the perspective divide.
...
Assembles a rotation out of individual quaternion components.
...
Emits a value chosen at random.
...
Swaps out one section of a string for a different one.
...
Adds a further Euler rotation on top of an existing Euler rotation.
...
Layers an extra rotation onto a rotation value already supplied.
