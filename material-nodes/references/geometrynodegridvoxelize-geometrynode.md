# Geometrynodegridvoxelize Geometrynode, Geometrynodegroup Geometrynode, Geometrynodeimageinfo Geometrynode, Geometrynodeimagetexture Geometrynode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: GeometryNodeGridVoxelize(GeometryNode); GeometryNodeGroup(GeometryNode); GeometryNodeImageInfo(GeometryNode); GeometryNodeImageTexture(GeometryNode); GeometryNodeImportCSV(GeometryNode); GeometryNodeImportOBJ(GeometryNode); GeometryNodeImportPLY(GeometryNode).
...
Densify a volume grid by converting its currently active tiles into individual voxels, eliminating the sparse storage.
...
class bpy.types. GeometryNodeGroup ( GeometryNode )
...
Read back metadata describing a given image.
...
Look up colors and values stored in an image texture.

extension

What happens to samples taken outside the image's native area (default 'REPEAT' )

- REPEAT
Repeat – Tile the image so it recurs across both axes.

- EXTEND
Extend – Stretch the outermost edge pixels outward.

- CLIP
Clip – Restrict to the image footprint, leaving anything outside it transparent.

- MIRROR
Mirror – Tile the image while alternately mirroring it on each axis.
...
How values get blended across neighboring pixels (default 'Linear' )

- Linear
Linear – Blend neighboring texels linearly.

- Closest
Closest – Snap to the nearest texel with no blending.

- Cubic
Cubic – Blend texels with a cubic curve.
...
Load geometry out of a CSV file on disk.
...
Load geometry out of an OBJ file on disk.
...
Bring in a point cloud stored in a PLY file.
