# Shadernodeseparatecolor Shadernode, Shadernodeseparatexyz Shadernode, Shadernodeshadertorgb Shadernode, Shadernodesqueeze Shadernode ...

> Blender Material Nodes and Images reference. Original prose; identifiers preserved verbatim.

## Overview

Covers: ShaderNodeSeparateColor(ShaderNode); ShaderNodeSeparateXYZ(ShaderNode); ShaderNodeShaderToRGB(ShaderNode); ShaderNodeSqueeze(ShaderNode); ShaderNodeSubsurfaceScattering(ShaderNode); ShaderNodeTangent(ShaderNode); ShaderNodeTexBrick(ShaderNode).
...
Breaks a color apart into its separate components, with a choice of color models.
...
Breaks a vector apart into separate X, Y, and Z channels.
...
Captures shading results such as lighting and shadow as a flat color. It is most often used in stylized, non-photorealistic workflows to post-process the result of BSDFs.
...
A multiple-scattering subsurface shader that models light sinking into a surface and scattering around beneath it.
Well suited to substances like skin, wax, marble, and milk
...
Produces a tangent vector for use with the Anisotropic BSDF.
...
A procedural texture that lays out a brick pattern.
...
Sets how far alternating rows of bricks are shifted sideways (in [0, 1], default 0.5)
...
Controls the spacing of offset rows. Setting it to 2 produces an alternating even/uneven row pattern. (in [1, 99], default 2)
...
Scales the brick width on the rows picked out by the Offset Frequency (in [0, 99], default 1.0)
...
Controls how frequently rows are made up of “squished” bricks (in [1, 99], default 2)
